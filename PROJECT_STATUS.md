...
- [x] 상단 캐릭터/대사 패널 폭을 50:50으로 조정
- [x] 상단 UI 좌우 파란 여백 제거, 헤더가 게임 영역 폭을 가득 채우도록 조정
- [x] 게임판 높이를 스테이지 비율에 맞춰 자동 측정하고 게임판 주변 여백을 약 4dp로 축소
- [x] 타일 스케일을 14px 정수배 고정에서 1px 단위로 완화해 가용 폭을 촘촘하게 사용
- [x] gameShell의 불필요한 파란 세로 빈 공간 제거, 남는 화면은 하단 조작부에 배분
- [x] 내부 통로 타일을 원본 확대 이미지 기준 따뜻한 크림/분홍 바탕 + 서로 평행한 픽셀 대각선 3줄로 변경
- [x] 상단 UI 높이 104dp, 캐릭터 패딩 3dp, 문구 22sp로 확대
- [x] 캐릭터 패널은 좌우 벽돌만, 대사 패널은 상하좌우 전체 벽돌 프레임으로 원본 규칙 적용
...
- [x] 원본 화면 기준 벽 전체 검은 외곽선 제거 및 내부 줄눈 적갈색화
- [x] 벽 바깥 베이지 영역을 각 타일 전체가 3×3 = 9개의 작은 정사각형으로 꽉 차고 각 칸 테두리가 보이도록 변경
- [x] 게임판 외곽의 진한 남색 테두리를 얇고 밝은 파란 프레임으로 조정
...
- [x] 하단 조작 UI를 사용자 참고 이미지 기준 휴대폰 키패드형으로 전면 교체
- [x] 좌상단 STAGE / 우상단 RESET / 중앙 상하좌우+OK 배치
- [x] 하단 초록 통화 키와 중앙 하단 버튼은 외형 유지, 오른쪽 빨간 전화키는 앱 종료 기능으로 사용

- [x] 내부 통로 타일을 원본형 비대칭 간격의 긴 대각선 3줄 패턴으로 변경, 각 선이 타일 전체를 가로지르도록 적용

- [x] 내부 통로 타일 대각선 굵기 축소, 1/2번 선은 경계 미접촉, 3번 선은 하단 방향으로 이동

- [x] 하단 키패드 오른쪽 아래 빨간 전화키에 앱 종료 기능 할당

- [x] 상단 중앙 작은 버튼과 하단 중앙 버튼은 모양만 유지하고 내부 기호/CANCEL 글자 제거

- [x] 내부 통로 타일을 사용자 제공 원본 대각선 타일 bitmap 리소스로 직접 렌더링하도록 변경
- [x] 빈 목표 집을 사용자 제공 집 이미지에서 배경 제거한 투명 PNG로 교체
- [x] 공이 들어간 뒤 box goal 애니메이션 프레임은 기존 그대로 유지

- [x] 공 넣기 전/후 목표 이미지를 56x56 nearest-neighbor 업스케일 PNG로 적용
- [x] BOX_2~BOX_11 공 목표 애니메이션을 업스케일 drawable 리소스로 전환
- [x] 사용자 제공 벽돌 이미지 기반 56x56 픽셀아트 벽돌 리소스로 교체
- [x] drawTile()을 bitmap 실제 source 크기 사용 방식으로 변경
