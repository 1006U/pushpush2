...
- 상단 캐릭터/대사 패널 폭을 50:50으로 조정
- 상단 UI 좌우 파란 여백 제거: 헤더가 게임 영역 폭을 가득 채우도록 조정
- 게임판 높이를 스테이지 비율에 맞춰 자동 측정하고 좌우/상하 여백을 4dp 수준으로 축소
- 14px 정수배 강제 대신 1px 단위 셀 크기를 사용해 게임판이 가용 폭을 거의 가득 사용하도록 조정
- gameShell의 남는 파란 세로 공간을 제거하고 남는 화면 높이는 조작부가 사용하도록 배치
- 내부 통로 타일을 원본 확대 이미지 기준 따뜻한 크림/분홍 바탕 + 서로 평행한 픽셀 대각선 3줄로 조정
- 상단 UI 높이를 92dp → 104dp, 캐릭터 패딩 8dp → 3dp, 문구 18sp → 22sp로 확대
- 캐릭터 패널은 좌우 벽돌만, 대사 패널은 상하좌우 전체 벽돌 프레임으로 원본 규칙 적용
- 게임판 바깥 원작형 파란 배경 적용
- STAGE/STEP 하단 파란 상태바 적용
- STAGE/STEP 상태바를 28sp 글자 / 60dp 높이로 확대하고 가로 스케일을 1.10으로 조정
- 하단 Anycall 스타일 D-pad를 기존 1.32 가로 비율에서 1.90으로 확대하고 작은 화면에서는 폭 안에 자동 제한
- 인접 벽을 셀별 박스가 아닌 연결형 벽돌 패턴으로 렌더링
- 원본 화면에 맞춰 벽 덩어리의 별도 검은 외곽선을 제거하고 줄눈을 짙은 적갈색으로 조정
- 벽 바깥 베이지 영역을 각 타일 전체가 3×3 = 9개의 작은 정사각형으로 꽉 차도록 나누고 각 칸 테두리를 표시
- 게임판 외곽 프레임을 원본처럼 얇고 밝은 파란선으로 완화
- 원본 피처폰형 베이지 게임판 점무늬 적용
- 플레이어 스프라이트 고대비 14×14 버전 적용
- 상황별 상단 캐릭터/문구 상태 전환 적용
- 클리어 연출이 보이도록 자동 다음 스테이지 페이드 약 0.9초
- 상단 리마스터 캐릭터 이미지 4종 적용 (START/MOVE/GOAL_SUCCESS/STAGE_CLEAR)
- 상단 캐릭터 패널 확대: 132×92dp
- 상황별 문구:
  - 시작: 영 차~! / 영 차~!
  - 이동: 헛! 둘~! / 헛! 둘~!
  - 공 넣기: 와우~!! / 짝 짝 짝 ..
  - 스테이지 클리어: 오~예~~ / 앗싸~~!!
- 인게임 14×14 캐릭터는 순한 큰눈 버전
...
- 하단 Anycall 스타일 D-pad 세로 기준을 84dp로 확대하고 조작 영역 높이를 248dp로 확장
...
- 하단 조작 UI를 사용자 참고 이미지 기준 휴대폰 키패드형으로 전면 교체
- 좌상단 STAGE, 우상단 RESET, 중앙 파란 패드에 상/하/좌/우, 중앙 OK 배치
- 하단 초록 통화 키와 중앙 하단 버튼은 외형만 유지하고, 오른쪽 빨간 전화키는 앱 종료 기능으로 사용
- 기존 66스테이지 반응형 축소/터치 반복/햅틱 동작 유지

- 내부 통로 타일 대각선 패턴을 원본 확대 이미지 기준으로 재조정: 비대칭 간격의 긴 대각선 3줄이 타일 전체를 가로지르도록 렌더링

- 내부 통로 타일 대각선 미세조정: 세 줄 굵기를 줄이고, 첫 번째/두 번째 대각선은 타일 경계에 닿지 않게 단축, 세 번째 대각선은 더 아래로 이동
